...
void proc_init(pcb_t *proc) {
     ...
     */
    pfn_t newPage = free_frame();
    fte_t *currFrame = (fte_t*)(mem + (newPage * PAGE_SIZE));
    memset(currFrame, 0, PAGE_SIZE);
    
    // mem[newPage * PAGE_SIZE] = 0;
     ...
     */
    proc->saved_ptbr = newPage;
    context_switch(proc);
    frame_table[newPage].protected = 1;
    frame_table[newPage].mapped = 1;
}
...
void context_switch(pcb_t *proc) {
    ...
    // current_process -> state = proc->state;
    // current_process->pid= proc->pid; 
}
...
uint8_t mem_access(vaddr_t address, char rw, uint8_t data) {
    ...
    uint16_t currOffset = vaddr_offset(address);
    ...
    */
    pfn_t currPageFrame = currPageEntry->pfn;
    frame_table[currPageFrame].timestamp = get_current_timestamp();
    /*
    ...
    */
    paddr_t physicalAddress = (paddr_t)((uint32_t)currPageFrame << OFFSET_LEN) | (uint32_t)currOffset;
    // uint8_t physicalAddress = (uint8_t)(mem + (currPageFrame * PAGE_SIZE) + currOffset);
    ...
    if (rw == 'r') {
        ...
        stats.writes++;
        currPageEntry->dirty = 1;
        // memset(&physicalAddress, data, sizeof(uint8_t));
...
void proc_cleanup(pcb_t *proc) {
    ...
    pfn_t frame = proc->saved_ptbr;
    pte_t *currPage = (pte_t*) (mem + (PAGE_SIZE * frame));
    
    /* Iterate the page table and clean up each valid page */
    for (size_t i = 0; i < NUM_PAGES; i++) {
        if (currPage[i].valid == 1) {
            // currPage[i].valid = 0;
            pfn_t currFrameNum = currPage[i].pfn;
            frame_table[currFrameNum].mapped = 0;
            frame_table[currFrameNum].protected = 0;
        }
        if (currPage[i].swap > 0) {
            swap_free(currPage + i);
        }   
    }
